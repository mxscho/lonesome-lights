...
PlayerHandler::PlayerHandler(ClientGame& game)
	: Updatable(),
	m_game(game),
					...
					for (unsigned int i_x = 0; i_x < m_game.get_map().get_tile_count_x(); ++i_x) {
						...
						if (tile.get_x() == static_cast<unsigned int>(floor(position.x)) &&
							...
							if (DestructibleRockTile* destructible_rock_tile = dynamic_cast<DestructibleRockTile*>(&tile)) {
								if (destructible_rock_tile->is_selected()) {	
									destructible_rock_tile->unselect();
									for (auto i_tile = m_game.m_opponent_selected_destructible_rock_tiles.begin(); i_tile != m_game.m_opponent_selected_destructible_rock_tiles.end(); ++i_tile) {
										if (*i_tile == destructible_rock_tile) {
											i_tile = m_game.m_opponent_selected_destructible_rock_tiles.erase(i_tile);
											return;
										}
									}
								} else {
									destructible_rock_tile->select(glm::vec3(1.0F, 1.0F, 1.0F));
									m_game.m_opponent_selected_destructible_rock_tiles.push_back(destructible_rock_tile);
								}
								return;
							} else if (CrystalTile* crystal_tile = dynamic_cast<CrystalTile*>(&tile)) {
								if (crystal_tile->is_selected()) {
									crystal_tile->unselect();
									for (auto i_tile = m_game.m_opponent_selected_crystal_tiles.begin(); i_tile != m_game.m_opponent_selected_crystal_tiles.end(); ++i_tile) {
										if (*i_tile == crystal_tile) {
											m_game.m_opponent_selected_crystal_tiles.erase(i_tile);
											break;
										}
									}
								} else {
									crystal_tile->select(glm::vec3(1.0F, 1.0F, 1.0F));
									m_game.m_opponent_selected_crystal_tiles.push_back(crystal_tile);
								}
								return;
							}
						}
...
void PlayerHandler::check_selected_unit() {
	if (m_game.m_player_id == 0) {
		...
		bool existing = false;
		for (auto& i_opponent_unit : m_game.get_opponent_units()) {
			if (i_opponent_unit.get() == m_selected_unit) {
				existing = true;
			}
		}
		if (!existing) {
