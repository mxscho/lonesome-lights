...
Movement::State::State(float position, float velocity)
	: position(position),
	velocity(velocity) {
}
...
Movement::State Movement::calculate(const Movement::State& initial_state, float initial_time, float delta_time) {
	...
	Movement::State state(
		initial_state.position + delta_time * delta_position_delta_time,
...
Movement::Derivative Movement::evaluate_derivative(const Movement::State& initial_state, const Movement::Derivative& input_derivative, float initial_time, float delta_time) {
    ...
    new_state.position = initial_state.position + delta_time * input_derivative.delta_position;
    ...
    output_derivative.delta_velocity = calculate_acceleration(new_state.position, new_state.velocity, initial_time + delta_time);
